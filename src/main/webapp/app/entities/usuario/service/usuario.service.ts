...
type RestOf<T extends IUsuario | NewUsuario> = Omit<T, 'birthDate'> & {
...
export class UsuarioService {
  ...
  protected convertDateFromClient<T extends IUsuario | NewUsuario | PartialUpdateUsuario>(usuario: T): RestOf<T> {
    return {
      ...usuario,
      birthDate: usuario.birthDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsuario: RestUsuario): IUsuario {
    return {
      ...restUsuario,
      birthDate: restUsuario.birthDate ? dayjs(restUsuario.birthDate) : undefined,
    };
  }
