...
type RestOf<T extends IEspecialista | NewEspecialista> = Omit<T, 'birthDate'> & {
...
export class EspecialistaService {
  ...
  addEspecialistaToCollectionIfMissing<Type extends Pick<IEspecialista, 'id'>>(
    ...
    if (especialistas.length > 0) {
      const especialistaCollectionIdentifiers = especialistaCollection.map(
        especialistaItem => this.getEspecialistaIdentifier(especialistaItem)!,
      );
  ...
  protected convertDateFromClient<T extends IEspecialista | NewEspecialista | PartialUpdateEspecialista>(especialista: T): RestOf<T> {
    return {
      ...especialista,
      birthDate: especialista.birthDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEspecialista: RestEspecialista): IEspecialista {
    return {
      ...restEspecialista,
      birthDate: restEspecialista.birthDate ? dayjs(restEspecialista.birthDate) : undefined,
    };
  }
